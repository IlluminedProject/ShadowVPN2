...
public static class RavenDbCertificates {
    private static readonly ILogger _logger = Log.ForContext(typeof(RavenDbCertificates));

    public static X509Certificate2 GenerateRootCa() {
        _logger.Information("Generating new Root CA certificate");
        ...
        _logger.Information("Root CA certificate generated successfully (Thumbprint: {Thumbprint})", cert.Thumbprint);
        return cert;
    }

    public static (RSA Rsa, CertificateRequest Request) GenerateCertificateRequest() {
        _logger.Information("Generating node certificate request");
    ...
    public static X509Certificate2 SignCertificateFromPem(string csrPem, X509Certificate2 signingCert) {
        ...
        var signedCert = csr.Create(signingCert, DateTimeOffset.UtcNow.AddDays(-1), signingCert.NotAfter,
            serialNumber);
        _logger.Information("CSR signed successfully (Thumbprint: {Thumbprint})", signedCert.Thumbprint);
        return signedCert;
    }

    public static void TrustCustomRootCa(X509Certificate2 certificate) {
        _logger.Information("Trusting custom Root CA (Thumbprint: {Thumbprint})", certificate.Thumbprint);
        using var store = new X509Store(StoreName.Root, StoreLocation.CurrentUser);
        store.Open(OpenFlags.ReadWrite);
        if (!store.Certificates.Contains(certificate)) {
            store.Add(certificate);
            _logger.Information("Custom Root CA added to the current user Root store");
        }
        else {
            _logger.Debug("Custom Root CA is already trusted");
        }
    }
}